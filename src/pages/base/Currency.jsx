...
import CurrencyHook from './hooks/Currency.hook';

const Currency = () => {
	const navigate = useNavigate();
    const { token } = useAppContext();
    const {
        currencies, loading,  page, rangePaginator, setPage,
        setIsLoading, totalPages, currentPage
    } = CurrencyHook({ token });
    ...
    return (
        <div>
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-y-4 mb-10 bg-secondary-100 p-8 rounded-xl">
                <div>
                    <h1 className="font-bold text-gray-100 text-xl">Listado de monedas</h1>
                </div>
            </div>
