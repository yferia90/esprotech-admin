...
import CustomerHook from './hooks/Customer.hook';
import CardCustomer from '../../components/CardCustomer';
import Tooltip from '../../components/Tooltip';
import ToastForm from '../../components/ToastForm';
import CustomModal from '../../components/CustomModal';
import CustomerSVG from './svg/Customer.svg';
...
const Customer = () => {
    ...
    const {
        ...
        mode, setMode, handlerEdit, isClient,
        page, rangePaginator, setPage,
        setIsUpdateCustomers, totalPages, currentPage
    } = CustomerHook({ token, companyId });
                ...
                <form className="mb-8">
                    <div className="flex items-center mb-8">
                      <div className="relative mb-4">
                          ...
                          <span className="text-secondary-100 ml-2">Subir foto</span>
                          <input                   
                            type="file"
                            name="file"
                            id="avatar"
        ...
        return (
            <>
                { customers && customers.length > 0 && (
                    <div className="hidden md:grid grid-cols-1 md:grid-cols-4 mb-10 p-4">
                        <h5>Nombre</h5>
                        <h5>Estado</h5>
                        <h5>Email</h5>
                ...
                {
                    customers  && customers.length > 0 ? customers.map((element, index) => (
                        <div key={index} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-center mb-4 bg-secondary-900 p-4 rounded-xl">
                            ...
                            <div>
                                <span className="py-1 px-2 bg-green-500/10 text-green-500 rounded-lg">
                                    {!element?.locked ? 'ACTIVO' : 'INACTIVO'}
                                </span>
                            </div>
                            <div>
                                {element?.email}
    ...
    return (
        <div>
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-y-4 mb-10 bg-secondary-100 p-8 rounded-xl">
                <div>
                    <h1 className="font-bold text-gray-100 text-xl">{`Listado de ${isClient ? 'clientes' : 'proveedores'}`}</h1>
                </div>
                <div className="flex items-center gap-4">
                    ...
                    {customers && customers.length > 0 && (
                        <Tooltip text="Vista en modo listado">
                            <button 
                                ...
                                className="bg-secondary-100/50 hover:bg-secondary-100 flex items-center gap-2 py-2 rounded-lg hover:text-primary transition-colors">
                    ...
                    <button
                        ...
                        type="button"
                    >
                        {`Nuevo ${isClient ? 'cliente' : 'proveedor'}`}
                    </button>
                </div>
            </div>
